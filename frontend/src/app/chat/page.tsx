...
const parseAnalysis = (analysisText: string): ParsedAnalysis => {
    const usedSentencesMatch = analysisText.match(/Used Sentences:\s*([\s\S]*?)\s*Reasoning:/);
    const reasoningMatch = analysisText.match(/Reasoning:\s*([\s\S]*?)\s*Confidence:/);
    const confidenceMatch = analysisText.match(/Confidence:\s*([\s\S]*)/);
